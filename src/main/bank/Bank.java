 ...
 * @CreateDate 2015-04-28 19:30:33
 * @version: 0.1
 */
public class Bank {
	...
	public int WelcomeMenu() {
		...
		System.out
				.println("/t0:Saver Account 1:Junior Account 2:Current Account.");
		int type = sc.nextInt();
		return type;
	}
	...
	public void CreateAccount() {
		...
		aDat.setBirth(s);
		System.out
				.println("Please Input Account Type: 0:Saver Account 1:Junior Account 2:Current Account");
		int accountType = sc.nextInt();
		s = sc.nextLine();
		while (accountType == 1) {
			if (!aDat.setType(accountType)) {
				System.out
						.println("You Can't Create Junior Account!\nPlease Input Other One.");
		...
		System.out
				.println("Please Input Your Personal Identification Number: \n(Six numbers)");
		s = sc.nextLine();
		while (s.length() != 6) {
		...
		if (accountManage.AddNewAccount(aDat) == true)
			System.out.println("Create Account Success! Your Account ID is : "
					+ aDat.getId() + " .");
		else
	...
	public void SuspendAccount() {
		...
		if (accountManage.CheckAccountStatByAccountID(accountID) == 0) {
			System.out
					.println("This Account has Already been Suspended! Return to Main Menu.");
			return;
		}
		...
		System.out
				.println("Input 'yes' to Confirm To Suspend Your Account ID = "
						+ accountID + ".");
		s = sc.nextLine();
		s.toLowerCase();
		if ("yes".equals(s)) {
			if (accountManage.SuspendAccountByID(accountID) == true)
				System.out.println("Suspend Account : " + accountID
						+ " Success!");
			else
	...
	public void CloseAccount() {
		...
		System.out.println("Input 'yes' to Confirm To Close Your Account ID = "
				+ accountID + ".");
		s = sc.nextLine();
		s.toLowerCase();
		if ("yes".equals(s)) {
	...
	public void DespositFunds() {
		...
		if (accountManage.CheckAccountStatByAccountID(accountID) == 0) {
			System.out
					.println("This Account has been Suspended! Return to Main Menu.");
			return;
		}
		System.out
				.println("Please Input Your Fund Type : \n 1.Cash \n 2.Cheque");
		fundType = sc.nextInt();
		s = sc.nextLine();
		while (fundType != 1 && fundType != 2) {
			System.out
					.println("Wrong Fund Type , Please Input Again.\n 1.Cash \n 2.Cheque");
			fundType = sc.nextInt();
			s = sc.nextLine();
		}
		switch (fundType) {
		...
		case 2:
			...
			if (amount < 0 || amount > 10000) {
				...
				System.out
						.println("Please Input the Date of Cheque:(eg:yyyy-mm-dd)");
				expire = sc.nextLine();
				if (expire.length() != 10) {
					...
					if (pendFundManage.AddPendingFund(pDat) == true)
						System.out
								.println("Add Pengding Fund Success! Your Pengding Fund ID is : "
										+ pDat.getId() + ".");
					else
	...
	public void ClearFunds() {
		...
		System.out.println("Clear the Funds which Expire is "
				+ sfm.format(date) + ".");
		...
		for (PendingFundDataBase pDat : listPendingFund) {
			...
			if (accountManage.ChangeFundByID(pDat.getAccountID(),
					pDat.getFund()))
	...
	public void WithdrawFunds() {
		...
		if (accountManage.CheckAccountStatByAccountID(accountID) == 0) {
			System.out.println("Your Account" + accountID
					+ " is Suspended! Return to Main Menu.");
			return;
		}
		System.out.println("Please Input Your Personal Identification Number:");
		pin = sc.nextLine();
		...
		System.out.println("user " + aDat.getName() + " , Your Account :"
				+ aDat.getId() + " is " + type[aDat.getType()]
				+ ".\nYour Credited Funds is " + aDat.getFund()
				+ " .\nOverdraft Limit is " + overdraft);
		System.out
				.println("Please Input the Amount of Funds you want to Withdaw:");
		amount = sc.nextDouble();
		s = sc.nextLine();
	...
	public void CheckFunds() {
		...
		aDat = accountManage.GetAccountByIDAndPIN(accountID, pin);
		listPendingFund = pendFundManage
				.GetPendingFundListByAccountID(accountID);
		System.out.println("user " + aDat.getName() + " , Your Account :"
				+ aDat.getId() + " is " + stat[aDat.getStat()]
				+ ".\nYour Credited Funds is " + aDat.getFund() + " .");
		if (listPendingFund.size() == 0) {
			System.out.println("You have no Pending Funds!");
		} else {
			System.out.println("You have " + listPendingFund.size()
					+ " Pending Funds!");
			for (PendingFundDataBase pDat : listPendingFund) {
				// pDat.disp();
				System.out.println("ID : " + pDat.getId() + " Funds : "
						+ pDat.getFund() + " Expire Date : " + pDat.getExpire()
						+ ".");
			}
		}
